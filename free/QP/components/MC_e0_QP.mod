...
subject to wbat_under1 {t in time, es in es_bus}:
 wbat[t] >=-dis_max*lambda1[t,es] + qbat[t]*lambda1dn[t] + dis_max*lambda1dn[t];
 
subject to wbat_under2 {t in time, es in es_bus}:
 wbat[t] >= ch_max *lambda1[t,es] + qbat[t]*lambda1up[t] - ch_max *lambda1up[t];
 
subject to wbat_over1 {t in time, es in es_bus}:
 wbat[t] <= ch_max *lambda1[t,es] + qbat[t]*lambda1dn[t] - ch_max *lambda1dn[t];
 
subject to wbat_over2 {t in time, es in es_bus}:
 wbat[t] <=-dis_max*lambda1[t,es] + qbat[t]*lambda1up[t] + dis_max*lambda1up[t];
 
 
subject to wqbat_under1 {t in time, es in es_bus}:
 wqbat[t] >=-dis_max*lambda2[t,es] + qqbat[t]*lambda1dn[t] + dis_max*lambda2dn[t];
 
subject to wqbat_under2 {t in time, es in es_bus}:
 wqbat[t] >= ch_max *lambda2[t,es] + qqbat[t]*lambda1up[t] - ch_max *lambda2up[t];
 
subject to wqbat_over1 {t in time, es in es_bus}:
 wqbat[t] <= ch_max *lambda2[t,es] + qqbat[t]*lambda1dn[t] - ch_max *lambda2dn[t];
 
subject to wqbat_over2 {t in time, es in es_bus}:
 wqbat[t] <=-dis_max*lambda2[t,es] + qqbat[t]*lambda1up[t] + dis_max*lambda2up[t];
