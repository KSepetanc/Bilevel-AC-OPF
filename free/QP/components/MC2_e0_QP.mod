...
subject to wch_under1 {t in time, es in es_bus}:
 wch[t] >= qch[t]*lambda1dn[t];
 
subject to wch_under2 {t in time, es in es_bus}:
 wch[t] >= ch_max*lambda1[t,es] + qch[t]*lambda1up[t] - ch_max*lambda1up[t];
 
subject to wch_over1 {t in time, es in es_bus}:
 wch[t] <= ch_max*lambda1[t,es] + qch[t]*lambda1dn[t] - ch_max*lambda1dn[t];
 
subject to wch_over2 {t in time, es in es_bus}:
 wch[t] <= qch[t]*lambda1up[t];
 
 
subject to wdis_under1 {t in time, es in es_bus}:
 wdis[t] >= qdis[t]*lambda1dn[t];
 
subject to wdis_under2 {t in time, es in es_bus}:
 wdis[t] >= dis_max*lambda1[t,es] + qdis[t]*lambda1up[t] - dis_max*lambda1up[t];
 
subject to wdis_over1 {t in time, es in es_bus}:
 wdis[t] <= dis_max*lambda1[t,es] + qdis[t]*lambda1dn[t] - dis_max*lambda1dn[t];
 
subject to wdis_over2 {t in time, es in es_bus}:
 wdis[t] <= qdis[t]*lambda1up[t];
